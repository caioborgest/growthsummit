...
const mapFromSupabase = (item: Record<string, unknown>, entityName?: string): Record<string, unknown> => {
  ...
  if (entityName === 'registrations') {
    if (item.paid_amount !== undefined) {
      result.amount = item.paid_amount;
      result.paid_amount = item.paid_amount;
      result.paidAmount = item.paid_amount;
    } else if (item.final_amount !== undefined) {
...
function getSelectFields(entity: string, projectId?: string, slug?: string): string {
  ...
  if (isGEProject(projectId, slug)) {
    if (entity === 'registrations') {
      return 'id,status,created_at,amount:paid_amount,paid_amount,user_id,name,email,phone,ticket_number,ticket_type,checked_in,check_in_at,qr_code,palestras_noturnas,cursos_selecionados,profiles!left:profiles!growth_experience_registrations_user_id_fkey(user_id,name,email,phone,company,city,state,role)';
    }
...
export function useData<T extends WithId>(initialData: T[] = [], entityName: string = 'registrations', options?: { realtime?: boolean, projectId?: string, filters?: Record<string, any> }) {
  ...
  const fetchData = useCallback(async (force = false, signal?: AbortSignal) => {
    ...
    try {
      ...
      if (supabaseError) {
        // Specific handling for GE transactions which might have restricted permissions
        if (entityName === 'transactions' && ((supabaseError as any).code === '42501' || supabaseError.message?.includes('permission'))) {
          logger.warn('[useData] Transactions table inaccessible (Permission Denied). Skipping...');
          setData([]);
          setIsLoading(false);
          isFetchingRef.current = false;
          return;
