...
// =========================================================================
// SCRIPT DE CADASTRO DE WEBHOOK - BANCO CORA (PRODUÇÃO)
// =========================================================================
const CORA_CLIENT_ID = 'int-6IyEG8vrP8SSY6FUzn4VW6'; // Coloque seu Client ID da Cora de Produção
...
async function registerWebhook() {
    try {
        console.log('1. Gerando Access Token (Client Credentials) em PRODUÇÃO...');
        ...
        console.log(`2. Cadastrando Webhook em PRODUÇÃO: ${WEBHOOK_URL}...`);
        const payload = { url: WEBHOOK_URL, events: ["INVOICE.PAID"] };
        
        const webhookRes = await axios.post(`${CORA_BASE_URL}/endpoints`, payload, {
            headers: { 
        ...
        console.log('\n✅ Webhook cadastrado na Cora (PRODUÇÃO) com Sucesso!');
        console.log(JSON.stringify(webhookRes.data, null, 2));
    } catch (error) {
        console.error('\n❌ Falha ao cadastrar Webhook (PRODUÇÃO):');
